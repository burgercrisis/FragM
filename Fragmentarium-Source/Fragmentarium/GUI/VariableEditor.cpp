...
QString VariableEditor::getSettings( bool p )
{
    QStringList l;
    if(p) {
        // only file name, don't need to spread foreign hard drive hierarchy
        l << tr("// Generated by: ") + mainWindow->currentFileName().split("/").last();
        l << tr("// Created on: ") + QDateTime::currentDateTime().toString();
    }
    for (int i = 0; i < variables.count(); i++) {
